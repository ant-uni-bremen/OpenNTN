...
specification. The model is defined mainly in Section 6 and especially 6.5 
of 3GPP TR38.811. 3GPP TR38.901 is used as the foundation for all models, 
with TR38.811 serving mainly as an extension.
"""
...
# the topology. The value specified when calling is ignored.")
        ...
        # TODO here the terminology should be cahnged, as moving end is not technically correct anymore.
        # However, as it is more of a cosmetikc issue at the moment and only influencing this limited part 
        # of the code, this will be done later
        ...
        # TODO Here it needs to be verified, that the angles and topology are intearcting correctly. The old 
        # implementation always chose the satellites plane, so that only one of the angles for either phi or
        # theta was adapted and thus either azimuth or zenith was zero. This should still be the case here,
        # however, a thourough verification is still to be done.
    ...
    def show_topology(self, bs_index=0, batch_index=0):
        ...
        ut_scaled_unit = "[m]"
        bs_scaled_unit = "[m]"
        ut_condition = tf.reduce_any(tf.greater(ut_loc, 10000.0))
        bs_condition = tf.reduce_any(tf.greater(bs_loc, 10000.0))
        if ut_condition:
            ut_loc = ut_loc/1000.0
            ut_scaled_unit = "[km]"
        if bs_condition:
            bs_loc = bs_loc/1000.0
            bs_scaled_unit = "[km]"
        ...
        x_label = str('x ' + ut_scaled_unit)
        y_label = str('y ' + ut_scaled_unit)
        z_label = str('z ' + bs_scaled_unit)
