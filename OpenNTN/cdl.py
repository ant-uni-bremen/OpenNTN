...
# However, parts of the code reuse implementations provided by the NVIDIA CORPORATION & AFFILIATES
#
...
"""
DISCLAIMER: THE CDL MODEL IS NOT YET FULLY DEFINED BY THE 3GPP. THUS, THE IMPLEMENTATION IS NOT YET PROVIDED. THIS FILE ONLY EXISTS TO 
BE A REFERENCE FOR A LATER IMPLEMENTATIONS, AS SOON AS THE STANDARD IS UPDATED. TO KEEP THE GENERAL CODE STRUCTURE IN THE DESIRED WAY,
WE HAVE ALREADY ADDED IT. CURRENTLY IT DOES NOT WORK AND IS MAINLY A COPY OF THE 38.901 VERSION
"""
"""Clustered delay line (CDL) channel model from 3GPP TR38.811 specification"""
...
class CDL(ChannelModel):
    ...
    def __call__(self, batch_size, num_time_steps, sampling_frequency):
        ...
        #TODO adjust delay spread and angles based on scaled angle in CDL model
    ...
    #TODO adjust scaling of dealys for NTN here as well
    @property
    def delays(self):
        r"""Path delays [s]"""
    ...
    #TODO adjust scaling of dealy spread for NTN here as well
    @property
    ...
    def _shuffle_angles(self, angles, distribution="gaussian"):
        # pylint: disable=line-too-long
        """
        ...
        if distribution == "gaussian":
            random_numbers = tf.random.normal(tf.shape(angles))  # Gaussian for azimuth
        elif distribution == "laplacian":
            random_numbers = tf.random.laplace(tf.shape(angles), loc=0.0, scale=1.0)  # Laplacian for zenith
    ...
    def _random_coupling(self, aoa, aod, zoa, zod):
        ...
        elevation.
        #TODO adjusted for 811
        #NOTE 5: For all considered scenarios the AOD/AOA distributions are modelled by a wrapped Gaussian distribution, the ZOD/ZOA distributions are modelled by a Laplacian distribution and the delay distribution is modelled by an exponential distribution. Page 83 38.811
        ...
        shuffled_zod : [batch size, num of BSs, num of UTs, maximum number of clusters, number of rays], tf.float
            Shuffled `zod`
        """
        shuffled_aoa = self._shuffle_angles(aoa, distribution="gaussian")
        shuffled_aod = self._shuffle_angles(aod, distribution="gaussian")

        # Shuffle zenith angles (ZoA and ZoD) using Laplacian distribution
        shuffled_zoa = self._shuffle_angles(zoa, distribution="laplacian")
        shuffled_zod = self._shuffle_angles(zod, distribution="laplacian")
