...
class AntennaElement:
    ...
    pattern : str
        Radiation pattern. Must be "omni", "38.901", "aperture", or "dlp".
    ...
    def field(self, theta, phi):
        """
        Field pattern in the vertical and horizontal polarization (TR 38.901 7.3-4/5)
        ...
        a = sqrt(self._radiation_pattern(theta=theta, phi=phi))
    ...
    def show(self):
        ...
        theta = tf.linspace(0.0, PI, 100)
        phi = tf.linspace(-PI, PI, 100)
        phi_grid, theta_grid = tf.meshgrid(phi, theta)
        a = self._radiation_pattern(theta_grid, phi_grid)
        # As the aperture pattern scales differently, the amplitude is transformed into the log domain 
        if self._pattern == "aperture":
            a = 50+tf.math.log(a,10)
            a = tf.where(a > 0, a, tf.cast(0.0, tf.float32))
        ...
        # For the aperture antenna, the gain is in dB
        if self._pattern == "aperture":
    ...
    def _radiation_pattern_circular_aperture(self, theta, phi, a = 10):
        """
        ...
        phi:
            Azimuth angle wrapped within (0,pi) [radian]

        a:
            Radius of antennas circular aperture in wavelengths
        """

        ka = a * 2 * math.pi
        r = tf.acos(tf.sin(theta) * tf.cos(phi))
        amplitude = tf.where(0.0 == r, tf.cast(1.0, tf.float32), 4 * tf.abs(tf.math.special.bessel_j1(tf.sin(r)*ka) / (ka * tf.sin(r))) ** 2)
        return tf.where(tf.abs(r) > 0.5*math.pi, tf.cast(0.0, tf.float32), amplitude)
    ...
    def _radiation_pattern_co_phased_DLp(self, theta, phi):
        ...
        Radiation pattern Table 2 in the mentioned document
        ...
        phi:
            Azimuth angle wrapped within (0,pi) [radian]
        """
        # Set parameters for calculations from Table 2
        theta_3db = 90.0
        SLAv = 25.0
        phi_3db = 90.0
        Am = 25.0
        ...
        A_EV = -tf.math.minimum(12 * ((theta_deg - 90.0) / theta_3db)**2, SLAv)
        A_EH = -tf.math.minimum(12 * (phi_deg/phi_3db) ** 2, Am)

        gain = -tf.math.minimum(-(A_EV + A_EH), Am)
        # Convert gain in linear
        gain = 10.0**(gain/10.0)
...
class PanelArray:
    ...
    Antenna panel array following the [TR38901] specification. Can be reused for the [TR38811] models.
    ...
    :ref:`CDL <cdl>`, :ref:`DenseUrban <dur>`, :ref:`Urban <urb>`, and :ref:`SubUrban <sur>`
    models.
    ...
    antenna_pattern : str
        Element radiation pattern, must be "omni", "38.901", "aperture", or "dlp"
