...
#
"""Rural channel model from 3GPP TR38.811 specification"""

import tensorflow as tf

from . import SystemLevelChannel
from . import RuralScenario


class Rural(SystemLevelChannel):
    # pylint: disable=line-too-long
    r"""Rural(carrier_frequency, ut_array, bs_array, direction, enable_pathloss=True, enable_shadow_fading=True, always_generate_lsp=False, dtype=tf.complex64)

    Rural channel model from 3GPP [TR38811]_ specification.

    Setting up a Rural model requires configuring the network topology, i.e., the
    UTs and BSs locations, UTs velocities, etc. This is achieved using the
    :meth:`~sionna.phy.channel.tr38811.Rural.set_topology` method. Setting a different
    ...
    The following code snippet shows how to setup an Rural channel model assuming
    an OFDM waveform:
    ...
    ...                       carrier_frequency = 3.5e9)
    >>> # Instantiating Rural channel model
    >>> channel_model = Rural(carrier_frequency = 3.5e9,
    ...                     ut_array = ut_array,
    ...                     bs_array = bs_array,
    ...                     direction = 'uplink')
    ...
    def __init__(self, carrier_frequency, ut_array, bs_array,
        ...
        # Rural scenario
        scenario = RuralScenario(carrier_frequency, ut_array, bs_array,
