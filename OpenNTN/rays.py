...
class RaysGenerator:
    ...
    def _compute_clusters_mask(self):
        ...
        num_clusters_los = tf.cast(scenario.num_clusters_los, dtype=tf.int32)
        num_clusters_nlos = tf.cast(scenario.num_clusters_nlos, dtype=tf.int32)
        ...
        num_clusters_max = tf.cast(tf.reduce_max([num_clusters_los, num_clusters_nlos,
            num_clusters_o2i]), dtype=tf.int32)
        num_clusters_max = tf.cast(tf.reduce_max([num_clusters_los, num_clusters_nlos]), dtype=tf.int32)
    ...
    def _azimuth_angles(self, azimuth_spread, rician_k_factor, cluster_powers,
        ...
        if angle_type == 'aod':
            ...
            if self._scenario.bs_loc[0,0,2] >= 160000:
                azimuth_spread = tf.zeros_like(azimuth_spread)
